...
static int setup_test_array(void **state) {
    ...
    *((size_t *) &fixture->element_size) = sizeof(int);
    ...
    fixture->array = array_create_dynamic(fixture->capacity);
...
static void test_basic_interface_creation_and_properties(void **state) {
    ...
    dsa_array_t *arr = array_create_dynamic(capacity);
    assert_non_null(arr);
...
static void test_random_access_get_set(void **state) {
    ...
    for (int i = 0; i < 5; i++) {
        values[i] = i * 10;
        assert_int_equal(array_push_back(arr, &values[i]), DSA_SUCCESS);
    }
    ...
    for (int i = 0; i < 5; i++) {
        new_values[i] = i * 100;
        assert_int_equal(array_set(arr, i, &new_values[i]), DSA_SUCCESS);
    }
...
static void test_back_interface_push_back(void **state) {
    ...
    int extra_value = 999;
    assert_int_equal(array_push_back(arr, &extra_value), DSA_SUCCESS);
    assert_int_equal(array_size(arr), fixture->capacity + 1);
    // 验证容量确实增加了
    assert_true(array_capacity(arr) > fixture->capacity);
    // 验证新添加的元素
    int *retrieved = (int *) array_get(arr, fixture->capacity);
    assert_non_null(retrieved);
...
static void test_back_interface_pop_back(void **state) {
    ...
    for (int i = 0; i < 3; i++) {
        ...
        array_push_back(arr, &int_values[i]);
    }
    assert_int_equal(array_size(arr), 3);
...
static void test_array_interface_type_info(void **state) {
    ...
    assert_int_equal(type, ARRAY_TYPE_DYNAMIC);
