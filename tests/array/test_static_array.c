...
static int setup_test_array(void **state) {
    ...
    *((size_t*)&fixture->element_size) = sizeof(int);
    ...
    fixture->array = array_list_create_static(fixture->buffer, fixture->capacity, fixture->element_size);
...
static void test_basic_interface_creation_and_properties(void **state) {
    ...
    dsa_array_list_t *arr = array_list_create_static(buffer, capacity, element_size);
    assert_non_null(arr);
...
static void test_random_access_get_set(void **state) {
    ...
    for (int i = 0; i < 5; i++) {
        int new_value = i * 100;
        assert_int_equal(array_list_set(arr, i, &new_value), DSA_SUCCESS);
    }
...
static void test_back_interface_push_back(void **state) {
    ...
    // 测试满容量时的push_back
    int extra_value = 999;
    assert_int_equal(array_list_push_back(arr, &extra_value), DSA_ERROR_CAPACITY_FULL);
    assert_int_equal(array_list_size(arr), fixture->capacity);
}
...
static void test_array_interface_type_info(void **state) {
    ...
    assert_int_equal(type, ARRAY_LIST_TYPE_STATIC);
...
static void test_null_pointer_handling(void **state) {
    ...
    // 测试空指针参数
    assert_null(array_list_create_static(NULL, 10, sizeof(int)));
    assert_null(array_list_create_static((void*)1, 0, sizeof(int)));
    assert_null(array_list_create_static((void*)1, 10, 0));
