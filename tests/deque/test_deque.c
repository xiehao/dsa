...
// 测试循环数组双端队列创建
static void test_circular_array_deque_creation(void **state) {
    (void) state;
    ...
    assert_int_equal(deque_get_type(deque), DEQUE_TYPE_CIRCULAR_ARRAY_LIST);
...
// 测试循环数组双端队列的扩容功能
static void test_deque_resize(void **state) {
    (void) state;
    dsa_deque_t *deque = deque_create(DEQUE_TYPE_CIRCULAR_ARRAY_LIST);

    // 添加超过初始容量的元素，测试扩容
    const int num_elements = 20; // 超过默认容量8
...
// 测试清空操作
static void test_deque_clear(void **state) {
    (void) state;
    dsa_deque_t *deque = deque_create(DEQUE_TYPE_CIRCULAR_ARRAY_LIST);
...
int main(void) {
    const struct CMUnitTest tests[] = {
        cmocka_unit_test(test_circular_array_deque_creation),
        ...
        cmocka_unit_test(test_deque_resize),
        cmocka_unit_test(test_deque_clear),
    };
